...
import type { ChessboardMap } from "../chessboard/models/chessboard.model";
import type { DroneRouteRepository } from "./droneRoute.repository";
import {
...
describe("droneRouteService", () => {
  ...
  const mockRoute: DroneRoute = {
    initialCoordinate: { x: "A", y: 1 },
    packageCoordinate: { x: "A", y: 2 },
    deliveryCoordinate: { x: "A", y: 3 },
    path: [
      { x: "A", y: 1 },
      { x: "A", y: 2 },
      { x: "A", y: 3 },
  ...
  const droneRouteRepository: DroneRouteRepository = {
    addRoute: () => Promise.resolve(true),
    getRoutes: () => Promise.resolve([mockRoute]),
  };
